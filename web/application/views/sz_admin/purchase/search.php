...
<div class="search_group">
    <form method="get">
        ...
        <div class="search_group_right float_clear initial">
            ...
            <div class="search">
                <label class="control_label" for="supplier_id">供应商</label>
                <select name="supplier_id"  class="m_inpt_border">
                    <option value=''>---请选择---</option>
                    <?php foreach ($supplier_data as $v): ?>
                   <option value='<?php echo $v['id']; ?>' <?php if ($this->input->get('supplier_id') == $v['id']): ?>selected='selected'<?php endif; ?>><?php echo $v['name']; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="search">
                <label class="control_label" >采购人</label>
        ...
        <div class="search">
        	<label class="contorl_label">采购人</label>
            <input type="text" name="person_like" value="<?php echo $this->input->get('person_like');?>" class="input_medium search_query"/>
        </div>
        <div class="search_butGroup float">
        	<button type="submit" class="btn btn_style1"><span>查询</span></button>
            <button type=""></button>
        </div>
    </form>

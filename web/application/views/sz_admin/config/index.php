...
<?php $this->load->view_part($this->_site_path . "/config/search") ?>

<!--列表版块-->
<form method="post" action="<?php echo base_url($this->_site_path . "/config/delete_all"); ?>">
    <div class="list_box">
        <table id="tb" class="table table-striped table-condensed table-bordered">
            ...
            <tbody>
                ...
                <?php foreach ($config_data as $single) { ?>
                <!--  鼠标悬停，整行变色，-->
                <tr >
                    <td>
                    ...
                    <?php foreach ($this->$modelname->list_array as $k => $v): ?>
                    <td onclick="javascript:window.location.href='<?php echo base_url($this->_site_path . "/config/view/{$single['id']}"); ?>';"><?php echo $single[$k]; ?></td>
                    <?php endforeach; ?>
                    
                    <td>
                        <a href="<?php echo base_url($this->_site_path . "/config/edit/{$single['id']}"); ?>">修改</a>
                    </td>
                </tr>
                <?php } ?>
            </tbody>

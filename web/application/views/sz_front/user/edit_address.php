...
<form name="form1" action="<?php echo base_url($this->_site_path . '/user/do_edit_addr')."/".$addr_info['address_id'];?>" method="post" onsubmit="return checkInfo();">
   <div class="user main" >
   	 <ul>
     	<li><span>收货人姓名</span><input class="txt_input" name="consignee" value="<?php echo $addr_info['consignee']?>"></li> 
     	<li>
     	<span>手机号码</span>
     	<input id="userTel" class="txt_input" name="mobile" value="<?php echo $addr_info['mobile']?>">
...
<div class="user_select">
  ...
  <select id="language" name="province">
      ...
      <option value="<?php echo $v['id'];?>" <?php if($v['id']==$addr_info['province']){echo 'selected';}?>><?php echo $v['province_name']?></option>
     <?php }?>
  </select>
  </div>
  </div>
  ...
  <div id="uboxstyle1" class="uboxstyle">
      <select id="language1" name="city" >
        <option value="">请选择</option>
        <?php 
  		$sql = "select * from user_city where pid={$addr_info['province']}";
  		$result = mysql_query($sql);
  		while($rs =  mysql_fetch_assoc($result)){
  		?>
         <option value="<?php echo $rs["id"]?>" <?php if($rs['id']==$addr_info['city']){echo 'selected';}?>><?php echo $rs["city_name"]?></option>
         <?php }?>      
      </select>  
  </div>
  </div>
  ...
  <div id="uboxstyle2" class="uboxstyle">
    <select id="language2" name="area">
        <option value="">请选择</option>    
        <?php 
  		$sql = "select * from user_area where pid={$addr_info['city']}";
  		$result = mysql_query($sql);
  		echo $sql;
  		while($rs =  mysql_fetch_assoc($result)){
  		?>
        <option value="<?php echo $rs["id"]?>" <?php if($rs['id']==$addr_info['area']){echo 'selected';}?>><?php echo $rs["area_name"]?></option>   
        <?php }?>    
    </select>  
    </div>
  </div>
           ...
           <span class="new_tbl_cell new_txt_w38">街道</span>
             <span class="new_tbl_cell">
                 <div class="new_post_wr">
                     <textarea name="address" id="address_where" rows="5" cols="30" title="" class="new_textarea"><?php echo $addr_info['address']?></textarea>
                  </div>
             </span>                        
    </div>

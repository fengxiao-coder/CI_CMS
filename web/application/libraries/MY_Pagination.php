...
class MY_Pagination extends CI_Pagination {
	...
	var $first_link			= '&lsaquo; 第一页';
	var $next_link			= '&gt;';
	var $prev_link			= '&lt;';
	var $last_link			= '最后一页 &rsaquo;';
	var $uri_segment		= 4;
	var $full_tag_open		= '<div class="dede_pages float_right"><ul>';
	...
	var $cur_tag_open		= '<a href="#" class="thisclass" >';
	var $cur_tag_close		= '</a>';
	var $next_tag_open		= '';
	var $next_tag_close		= '';
	var $prev_tag_open		= '';
	...
	function create_links()
	{
		...
		if  ($this->prev_link !== FALSE)
		{
			if ( $this->cur_page != 1 ) {
				...
				$output .= $this->prev_tag_open.'<a '.$this->anchor_class.' disabled="true">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}	
		}
		...
		if  ($this->cur_page > ($this->num_links + 1))
		{
			...
			$output .= $this->first_tag_open.'  <a> ... </a>  '.$this->first_tag_close;
			//echo $output ; 
		}
		...
		if ( ($this->cur_page + $this->num_links) < $num_pages)
		{
			...
			$output .= $this->last_tag_open.' <a> ... </a> '.$this->last_tag_close;
		...
		if ($this->next_link !== FALSE)
		{
			if ($this->cur_page < $num_pages) {
				...
				$output .= $this->next_tag_open.'<a '.$this->anchor_class.'disabled="true">'.$this->next_link.'</a>'.$this->next_tag_close;
			}
		}
		
		//添加输入页码功能
        $output .='<span>共'.$num_pages.'页</span><span>到第<input id="page_jump" value="'.$this->cur_page.'"/>页</span><a class="thisclass" href="javascript:window.location=\''.$this->base_url.$this->prefix.'/\'+document.getElementById(\'page_jump\').value'.'+\''.$this->suffix.'\';; doane(event);">确定</a>';
